    ...
    if service == "logs":
        if output:
            ...
            for item in usage_report_sorted:
                click.echo(
                    f"ARN: {item.arn}, Size: {item.get_size_in_gb()} GB, Cost: ${item.get_cost()}, Retention: {item.retention_in_days}, Created: {item.created_time}, Region: {item.region}")

            with open(output, "w") as f:
                f.write("ARN,Size,Retention,Cost,Created,Region\n")
                for item in usage_report_sorted:
                    f.write(
                        f"{item.arn},{item.get_size_in_gb()},{item.retention_in_days},{item.get_cost()},{item.created_time},{item.region}\n")
